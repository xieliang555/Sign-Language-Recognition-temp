...
#     def __init__(self, tgt_vocab_size, d_model, nhead, nhid, dropout, activation, nlayer):
#         super(TransformerEncoder, self).__init__()
#         self.d_model = d_model
        
#         self.pe = PositionalEncoding(d_model)
#         self.dropout = nn.Dropout(dropout)
#         encoder_layer = nn.TransformerEncoderLayer(d_model, nhead, nhid, dropout, activation)
...
class Transformer(nn.Module):
    ...
    def get_subsequent_mask(self, tgt):
        ...
        mask = mask.masked_fill(mask==1.0, float('-inf')).masked_fill(mask==0.0, float(0.0))
        return mask
...
class Seq2Seq(nn.Module):
    ...
    '''
    def __init__(self, nclip, encoder, decoder, device):
    ...
    def forward(self, x, tgt):
        # encoder_out: [T, N, E]
        encoder_out = torch.zeros(self.nclip, x.size(0), self.decoder.d_model).to(self.device)
        x = x.chunk(self.nclip, 2)
        for idx, clip in enumerate(x):
            encoder_out[idx,:,:] = self.encoder(clip)
